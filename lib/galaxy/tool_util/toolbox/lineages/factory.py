...
class LineageMap:
    ...
    def get(self, tool_id: str) -> ToolLineage | None:
        ...
        if tool_id not in self.lineage_map:
            ...
            # lineage from the toolbox this map belongs to, so it is available
            # while that toolbox is still being built and a reload reads the
            # tools it is loading rather than the ones it replaces.
